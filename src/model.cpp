...
Vertex::Vertex(GLfloat _x, GLfloat _y, GLfloat _z)
{
                  x[0] = _x;
                  x[1] = _y;
                  x[2] = _z;
...
void Model::load(const char *filename)
{
    ...
    while(getline(file, s)) {
        ...
        if(cmd == "mtllib") {
            sstr >> matfile;
            //matfile = std::string("data/") + matfile;
...
void Model::make_list()
{
    ...
    //MessageBox(NULL, "Start listy", "DEBUG", MB_OK);
    ...
    glEnd();
    //MessageBox(NULL, "Koniec listy", "DEBUG", MB_OK);
